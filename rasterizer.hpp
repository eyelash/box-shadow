...
template <class F> void draw_graph(Pixmap& pixmap, int x_, int y_, int width, int height, const Color& color, F f) {
	...
	for (int x = 0; x < width; ++x) {
		...
		const float x0 = x;
		const float x1 = x + 1;
		float y0 = height - f(x0 / width) * height;
		float y1 = height - f(x1 / width) * height;
		if (y0 > y1) std::swap(y0, y1);
		const float delta_y = length(Point(x0, y0) - Point(x1, y1)) * (line_width * .5f);
		const float y2 = y0 - delta_y;
		const float y3 = y1 - delta_y;
		const float y4 = y0 + delta_y;
		const float y5 = y1 + delta_y;
		for (int y = 0; y < height; ++y) {
			const float y6 = y;
			const float y7 = y + 1;
			if (y2 > y7 || y5 < y6) {
			...
			const float area = compute_area(y6, y7, y4, y5) - compute_area(y6, y7, y2, y3);
